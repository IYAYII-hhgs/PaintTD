...
/**
 * 可拷贝可序列化的组件
 * <p>组件默认情况完整参与深度序列化且不需要refill, 也可继承{@link com.badlogic.gdx.utils.Json.Serializable}自定义序列化, 并根据需求refill
 * <p>不参与序列化的组件, 使用{@link com.artemis.annotations.Transient}注解
 * <p>对于{@code transient}的组件及其字段, 反序列化中将按{@link io.blackdeluxecat.painttd.content.EntityType}的定义重建. 不会自动从实体类型拷贝组件默认值, 需重写{@link #refill(CopyableComponent)}
 */
